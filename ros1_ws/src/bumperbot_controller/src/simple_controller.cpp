...
SimpleController::SimpleController(const ros::NodeHandle &nh,
                                   double radius,
                                   double separation)
                                  : nh_(nh), wheel_radius_(radius), 
                                  wheel_separation_(separation), left_wheel_prev_pos_(0.0), right_wheel_prev_pos_(0.0),
...
void SimpleController::jointCallback(const sensor_msgs::JointState &state)
{
    ...
    // ROS_INFO_STREAM("linear velocity: "<<linear<<"angular velocity: "<<angular);
